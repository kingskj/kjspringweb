...
- 수정 후 최소 검증:
  ...
  - 실패 시 원복/수정 방향 명확화
- 사용자 검증 선호:
  - 단위 테스트 파일을 새로 만들어 검증하는 것보다, 대상 서버와 필요한 연동 서버를 실제로 기동한 뒤 HTTP 요청, trace/log, DB/meta를 대조하는 실기동 E2E 검증을 우선한다.
  - 단위 테스트는 빌드 안정성 및 좁은 회귀 확인용 보조 수단으로 사용한다.
  - TurtlePick agent, javaagent, ASM, retransform, Spring proxy, 서버-엔진 통신처럼 런타임 결합이 핵심인 변경은 실기동 검증 없이는 완료로 보지 않는다.
